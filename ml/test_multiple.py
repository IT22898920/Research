"""
Test model with multiple random images from both classes
"""
...
def load_model():
    ...
    # Use best_model.keras - best validation accuracy!
...
def main():
    ...
    print(f"Model: {model_info['model_name']} | Accuracy: {model_info['test_accuracy']:.2%}\n")
    ...
    for f in random.sample(mite_files, min(5, len(mite_files))):
        ...
        if result['class'] == 'coconut_mite':
            mite_correct += 1
        print(f"  {f[:20]:20} -> {result['class']:12} ({result['confidence']:.1%}) [{status}]")

    print(f"\n  Mite Accuracy: {mite_correct}/5 = {mite_correct/5:.0%}")

    print("\n" + "=" * 60)
    print("Testing 5 HEALTHY images:")
    print("=" * 60)
    healthy_correct = 0
